...
- **timer**: 사용 토글 / 기본 시간 스테퍼(기기를 안 건드려 포트 잠금 걱정이
  ...
  남은 시간 + [+5분][−5분][기본값]. 그게 전부다 —
  **설명 문구는 사용자 요청으로 전부 제거**했다(컨트롤만으로 명확). 다시
  넣지 말 것. 기능 설명은 README 몫이다.
  메인에는 `타이머: 12:34 남음` 행(무장 중 accent, 60초 이하 urgent)과
  만료 안내(5분간)가 뜬다.
  ⚠️ **끄는 수단은 '타이머 사용' 토글 하나뿐** — 여기에 [해제]를 또 두면
  "토글은 켜짐인데 타이머는 없음"이 생겨 둘이 어긋난다(사용자 지적으로
  제거). 토글을 끄면 무장 중이던 타이머도 함께 사라지고, 토글을 켜면
  이전 보류 흔적을 지워 반드시 다시 걸린다. `suppressed`(이번 출력만
  건너뛰기)는 이제 `psu timer disarm` CLI로만 만들어진다. GUI에는 그
  상태를 위한 UI가 없고(메인 행에 '이번엔 해제됨' 표시만), 토글을 껐다
  켜면 풀린다.
