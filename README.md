...
- 클릭 팝업 (뷰 4개):
  ...
  - **타이머**: 출력 자동 차단 — 켜두고 잊는 사고 방지. 켜고 끄는 건
    '타이머 사용' 토글 하나, 무장 중 남은 시간은 메인에 표시되고
    +5분/−5분/기본값으로 조절
...
psu timer extend 5         # 무장 중 남은 시간 조절
psu timer disarm           # 이번 출력 동안만 건너뛰기 (GUI에는 없는 기능)
...
끄는 수단은 '타이머 사용' 토글 하나다 — 끄면 무장 중이던 타이머도 함께
사라지고, 켜면 반드시 다시 걸린다. (`psu timer disarm` 은 타이머를 켜둔 채
이번 출력만 건너뛰는 CLI 전용 탈출구다. GUI에는 없고, 토글을 껐다 켜면
풀린다.)
